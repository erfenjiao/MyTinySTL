...
namespace mytsl
{
    ...
    // list 的迭代器设计
    template<class T>
    struct last_iterator : public mystl::<mystl::bidirectional_iterator_tag, T>
    {
        typedef   T                value_type;
        typedef   T*               pointer;
        typedef   T&               reference;
        tyepdef typename node_traits<T>::base_ptr      base_ptr;
        tyepdef typename node_traits<T>::node_ptr      node_ptr;
    ...
    class list
    {
        public:
            ...
            typedef mystl::allocator<list_const_iterator<T>>    base_allocator;
            typedef mystl::allocator<list_iterator<T>>          node_allocator;
        ...
        public:
            ...
            {
                copy_assgin(first , last);
            }

            void assgin(std::initializer_list<T> ilist)
            {
                copy_assgin(ilist.begin() , ilist.end());
            }
            ...
            vois push_front(const value_type& value)
            {
                THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
